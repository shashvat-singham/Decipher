...
def improve_mapping(encrypted_message, mapping, dictionary):
    ...
    while calculate_accuracy(decrypt_substitution(encrypted_message, mapping), dictionary) < 0.9:
        i = 0
        if (time.time() - start_time) >= 10: #Restart every 10s
            print("Stuck after",iter,"iterations, Restarting!")
            substitution(encrypted_message, dictionary)
        while i < 100:
            iter = iter + 1
            i = i + 1
...
if __name__ == '__main__':
    encrypted_message = "Pbh wmqk kxrkekc rwk useor dwmzyke bu rwk dmqko. Mo pbh dmx okk rwkek so xbrwsxt bu sxrkekor sx rwk dwmzyke. Obzk bu rwk fmrke dwmzykeo nsff yk zbek sxrkekorsxt rwmx rwso bxk, s mz okesbho. Rwk dbck hokc ube rwso zkoomtk so m oszjfk ohyorsrhrsbx dsjwke sx nwsdw cstsro wmqk ykkx owsurkc yp 4 jfmdko. Ube rwso ebhxc jmoonbec so tsqkx ykfbn, nsrwbhr rwk ahbrko. nhXp85Qbcc"
