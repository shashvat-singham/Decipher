...
'''
def improve_mapping(encrypted_message, mapping, dictionary, frozen_indices):
    ...
    for i in range(26):
        if i not in frozen_indices:
            choices.append(i)
    while calculate_accuracy(decrypt_substitution(encrypted_message, mapping), dictionary) < 0.85:
        i = 0
        if (time.time() - start_time) >= 15: #Restart every 15s
            print("Stuck after",iter,"iterations, Restarting!")
            return
        while i < 100:
            iter = iter + 1
            i = i + 1
            index1, index2 = random.choice(choices), random.choice(choices)
...
def substitution(encrypted_message, dictionary):
    ...
    password_encryption = encrypted_message.split()[-2][:-1]
    password_real = "password"
    for i in [0,1,3,4,5,6,7]:
        mapping[ord(password_encryption[i])-97] = ord(password_real[i])-97
        mapped_indices.append(ord(password_encryption[i])-97)
        unmapped_chars.remove(ord(password_real[i])-97)
        frozen_indices.append(ord(password_encryption[i])-97)
    ...
    mapping[ord(most_frequent[2])-97] = ord('e')-97
    frozen_indices.append(ord(most_frequent[0])-97)
    frozen_indices.append(ord(most_frequent[1])-97)
    frozen_indices.append(ord(most_frequent[2])-97)
    mapped_indices.append(ord(most_frequent[0])-97)
    mapped_indices.append(ord(most_frequent[1])-97)
    mapped_indices.append(ord(most_frequent[2])-97)
        ...
        for i in range(26):
            if i not in mapped_indices:
                try:
                    ...
                    return
        if flag == 1:
            improve_mapping(encrypted_message, mapping, dictionary, frozen_indices)
