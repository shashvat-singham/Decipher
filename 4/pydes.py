...
def bit_array_to_string(array): #Recreate the string from the bit array
    strs = [''.join([str(x) for x in _bytes]) for _bytes in  nsplit(array,8)]
    print(strs)
    res = ''.join([chr(int(y,2)) for y in strs])
    return res
...
if __name__ == '__main__':
    ...
    for i in range(len(r)):
        print(str(hex(ord(r[i])))[2:], end="")
        # print(bin(ord(r[i])))
    print()
    l = [i for i in range(1,65)]
    l = d.permut(l, PI)
    # print(d.permut(l[:32], inv_perm))
